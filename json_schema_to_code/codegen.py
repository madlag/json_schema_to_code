...
class CodeGenerator:
    ...
    def _handle_union_type_with_defaults(
        self, type_info, union_key: str, is_required: bool
    ) -> dict:
            ...
            if type == "array":
                if isinstance(type_info["items"], dict):
                    item_type_info = self.translate_type(
                        type_info["items"], is_required=True
                ...
                if len(type) == 1:
                    ...
                    typeNames = [
                        self.translate_type({"type": t}, is_required=True)["type"] for t in type
                    ]
                    type = self.union_type(typeNames)
            else:
                # Handle inline objects with properties
                if type == "object" and "properties" in type_info:
                    return self._handle_inline_object(type_info, is_required)
    ...
    def _handle_inline_object(self, type_info, is_required=True):
        """Handle inline object definitions by generating nested classes"""
        # Generate a unique class name for this inline object
        inline_class_name = self._generate_inline_class_name(type_info)
    ...
    def _generate_inline_class_name(self, type_info):
        """Generate a meaningful class name for an inline object"""
        # Try to use title if available
        if "title" in type_info:
            return self._to_pascal_case(type_info["title"])

        # Try to extract meaningful terms from description
        if "description" in type_info:
            desc = type_info["description"].strip()
            # Look for key terms that indicate what this object represents
            key_terms = [
                "problem",
                "state",
                "result",
            ...
            for term in key_terms:
                if term.lower() in desc.lower():
                    return self._to_pascal_case(term)

            # Look for "X being Y" or "X of Y" patterns
            import re

            patterns = [
                r"(\w+)\s+being\s+\w+",  # "problem being solved"
                r"(\w+)\s+of\s+\w+",  # "state of computation"
                r"(\w+)\s+for\s+\w+",  # "data for processing"
            ]
            for pattern in patterns:
                ...
                if match:
                    ...
                    if len(word) > 2 and word.lower() not in ["the", "and", "for", "with"]:
                        return self._to_pascal_case(word)

        # Fallback: generate based on properties
        if "properties" in type_info:
            props = list(type_info["properties"].keys())
            if props:
                # Use first property name as basis, but make it more meaningful
                first_prop = props[0]
                if len(props) == 1:
                    return self._to_pascal_case(first_prop) + "Data"
                else:
                    # Try to find a common theme in property names
                    if any("result" in prop.lower() for prop in props):
                        return "ResultData"
                    elif any(
                        "config" in prop.lower() or "setting" in prop.lower() for prop in props
                    ):
                        return "Configuration"
                    else:
                        return self._to_pascal_case(first_prop) + "Object"

        # Final fallback
        return f"InlineObject{len(self.class_info)}"
