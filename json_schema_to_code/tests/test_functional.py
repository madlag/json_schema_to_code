...
def load_all_test_cases():
    ...
    functional_dir = Path(__file__).parent / "test_data" / "functional"
    test_cases = []
...
def test_functional_generation(test_case):
    ...
    if "expected_python" in test_case:
        ...
        for expected in test_case["expected_python"]:
            ...
            ), f"Expected pattern '{expected}' not found in Python output"
    ...
    if "expected_cs" in test_case:
        ...
        for expected in test_case["expected_cs"]:
            assert (
                expected in generated_code
            ), f"Expected pattern '{expected}' not found in C# output"
    ...
    if "expected_contains" in test_case:
        ...
        for pattern in test_case["expected_contains"]:
            ...
            ), f"Expected pattern '{pattern}' not found in {language} output"
    ...
    if "expected_not_contains" in test_case:
        ...
        for pattern in test_case["expected_not_contains"]:
            assert (
                pattern not in generated_code
            ), f"Unexpected pattern '{pattern}' found in {language} output"
